...
`.` 으로 시작하는 폴더는 Unity가 보지 않으므로, 이 껍데기가 **소비 프로젝트의 에셋 목록에는**
올라오지 않습니다.

> **배포본에는 이야기가 다릅니다.** Unity가 임포트하지 않는 것과 tarball에 들어가지 않는 것은
> 다른 문제입니다. 아무것도 하지 않으면 이 폴더는 UPM 배포본에 **그대로 실려 나갑니다.**
> 그것을 막는 것은 저장소 루트의 `.npmignore` 입니다. 바꾼 뒤에는 `npm pack --dry-run` 으로
> 목록을 확인하십시오 — CI가 그 검사를 자동으로 돌리지만, 손으로 한 번 보는 편이 빠릅니다.
