...
## 5. 시행 결과 (2026-08-10, v0.8.0)

로드맵 세 항목을 순서대로 시행했습니다. 아래는 시행 전후로 실제 세어 본 값입니다.

| 항목 | 시행 전 (v0.7.0) | 시행 후 (v0.8.0) |
|---|---|---|
| Editor 코드 | 29파일 / 5,858줄 | **40파일 / 6,694줄** |
| 검사 | 8파일 / 1,250줄, **72건** | **12파일 / 1,774줄, 98건** |
| 인터페이스 | **0** | **1** (`ICsvAssetGateway`) |
| `AssetDatabase`를 직접 부르는 파일 | 17 | **7** (전부 에디터 셸) |
| 최대 파일 | `GoogleSheetSync.cs` **737줄** | `CsvPipelineWindow.cs` **557줄** |
| 40줄 초과 메서드 | 15 | **10** |

**줄 수는 늘었고, 그게 맞습니다.** 경계를 만들면 계약(인터페이스)과 두 번째 구현이 새로
생기므로 총량은 늡니다. 줄인 것은 총량이 아니라 **한 자리가 지고 있던 책임의 수**입니다.

### 1순위 — 무엇이 실제로 달라졌나

굽기 규칙 11건이 **임시 폴더 없이** 돕니다. 통합 검사에는 실제 AssetDatabase에서만 드러나는
셋만 남겼습니다 — 디스크 기록·지운 자리 재생성·GUID 참조 훑기. 나머지를 메모리로 옮긴 덕에
검사끼리 얽히는 사고는 **재발할 자리가 없어졌습니다.** 폴더를 공유하지 않으니까요.

`CsvAssetPipeline`의 공개 API는 한 줄도 바꾸지 않았습니다. 호출부 churn 없이 뒤만 갈렸고,
"같은 일을 하는 두 가지 방법"(§1-3에서 지적한 `CsvReader`의 두 진입점 같은 것)을 새로
만들지 않았습니다.

### 2순위 — 예상하지 못했던 소득

가르는 도중, 대조 실패 안내가 **색인 크기를 "줄 수"라고 부르고 있던 것**을 찾았습니다.
색인은 겹친 식별자를 접으므로, 하필 그 안내가 열리는 상황(식별자가 겹치거나 비어 있는 표)에서
양쪽 다 같은 숫자가 나와 아무것도 알려 주지 못했습니다. 737줄 안에 있을 때는 보이지 않았고,
순수 함수로 떼어 내 검사를 쓰자 바로 드러났습니다.

"받기"와 "비교만"이 같은 절차를 각자 적어 두던 것도 `SheetFetch` 하나로 합쳐졌습니다.
이제 한쪽만 고쳐 다른 쪽이 어긋나는 일이 성립하지 않습니다.

### 3순위 — 계약이 된 것

`BakeEach`가 진행·취소·집계·정리를 맡고, 파생은 "단위 하나를 어떻게 굽는가"만 답합니다.
**취소 시 정리를 건너뛰는 규칙이 한 곳에 있고, 검사가 그것을 지킵니다**
(`CsvBakeSkeletonTests.멈추면_정리하지_않는다`).

부수적으로 `CancelCurrentRun`이 생겼습니다. 굽는 중에 더 진행해 봐야 결과가 틀어질 조건을
알아챈 파생이 안전하게 멈출 수 있고, 그때도 정리는 자동으로 건너뜁니다.

### 남은 것

§3-1의 `[CsvAsset]` 전환과 그 뒤의 `CsvReader.Read` 제거, `FindReferenced` 캐시(실측 먼저).
순위에 들지 않은 이유는 위와 같고, 이번 시행으로 바뀌지 않았습니다.
