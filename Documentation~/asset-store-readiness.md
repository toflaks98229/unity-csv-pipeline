...
> **⚠️ 이 문서는 2026-08-23 부로 전제가 달라졌습니다.**
> 유통을 **비공개 저장소 + 상용 EULA(직접 판매)** 로 정했습니다. 라이선스는 MIT 에서 상용으로
> 바뀌었고(0.14.0~), `package.json` 의 저장소 URL 셋을 빼 Package Manager 가 패키지 안의 문서를
> 열도록 했습니다. 아래 §2 의 Asset Store 제출 요건은 **그 채널로 낼 경우에만** 유효하며,
> §4 의 경쟁 지형·§8 의 "낼 것인가"는 조사 시점(v0.1.1)의 기록입니다.


각 항목의 **근거**를 표시합니다. `측정` = 이 저장소에서 실제로 돌려 본 것,
`인용` = Unity 공식 문서·스토어 페이지에서 확인한 것, `판단` = 조사에 기반한 내 의견입니다.

> **진행 상황 (v0.6.0 기준)** — §5의 기능 격차 **F1~F6 전부**, 제출 요건 **B1·B3**,
> 위험 **R1·R2·R4**, UI/UX **U1~U7 전부** 를 해소했습니다. 검사 72건은 배치 모드에서 전부 통과합니다.
> 아래 본문은 조사 시점(v0.1.1)의 기록이며, 해소된 항목에는 ✅ 를 붙였습니다.
>
> **남은 것**: 제출 요건 **B2**(영문 문서)와 위험 **R3**(네트워크 고지 — 스토어 설명란 작성 시)뿐입니다.
> 둘 다 "낼 것인가"가 정해진 뒤의 일입니다.
...
| ✅ **U2** | **대시보드 창** | ~~없음~~ → `CSV 파이프라인 창`(표·시트·설정 갈래). **'마지막 임포트 시각'만 빼고** 넣었습니다 — 상태를 따로 저장해야 하는데, 계획이 이미 '지금 구우면 무엇이 달라지는가'를 보여 주므로 더 나은 답입니다 | — | 중 |
| ✅ **U3** | 오류의 위치 | ~~Console 텍스트뿐~~ → 리포트가 줄·열을 달고, 미리보기에 '찾기' 버튼 | — | 중 |
| ✅ **U4** | 첫 실행 안내 | ~~조용히 동작~~ → 미리보기 창의 빈 상태가 붙이는 두 길과 CSV 루트를 안내 | — | 하 |
| ✅ **U5** | 연동 설정 인스펙터 | ~~원시 필드 나열~~ → 시트 ID·gid·대상 표·마지막 동기화·비교 버튼 | — | 하 |
| ✅ **U6** | 임포트 진행·취소 | ~~없음~~ → 200행 이상에서 진행바+취소. **취소 시 정리는 건너뜀** | — | 하 |
| ✅ **U7** | Undo | 지원하지 않음이 의도임을 README에 이유와 대안까지 명시 | — | 하~중 |
