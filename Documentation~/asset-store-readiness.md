...
> **진행 상황 (v0.5.0 기준)** — §5의 기능 격차 **F1~F6 전부**, 제출 요건 **B1·B3**,
> 위험 **R1·R2·R4**, UI/UX **U1(미리보기)·U3(오류 위치)** 를 해소했습니다.
> 아래 본문은 조사 시점(v0.1.1)의 기록이며, 해소된 항목에는 ✅ 를 붙였습니다.
>
> **남은 것**: 제출 요건 **B2**(영문 문서), 위험 **R3**(네트워크 고지 — 스토어 설명란 작성 시),
> UI/UX **U2**(대시보드 창)·**U4~U7**.
...
| 자동화 테스트 | ~~0개~~ → **EditMode 72건, 배치 모드 전부 통과** | 측정 |
...
| # | 요건 | 현재 | 근거 |
|---|---|---|---|
| ✅ **B1** | 신규 제출은 **Unity 2022.3 이상** | ~~`2021.3`~~ → `2022.3` | 인용 |
...
| # | 항목 | 내용 | 근거 |
|---|---|---|---|
| ✅ **R1** | **Domain Reload 비활성 지원** (Unity 6.6+ 요구) | 걸린 채 남으면 기능이 죽는 래치를 편집 모드 복귀 시 품. 나머지 정적은 캐시라 자체 무효화가 있음 | 인용 + 측정 |
| ✅ **R2** | 상시 에디터 콜백 | 연동 설정이 없으면 아예 걸지 않음. 생기면 임포트 통지가 걸고, 지워지면 뗌 | 측정 |
...
| # | 항목 | 현재 | 있어야 할 것 | 노력 |
|---|---|---|---|---|
| ✅ **U1** | **미리보기 / 드라이런** | ~~없음~~ → `Tools ▸ CSV Pipeline ▸ 미리보기`. 필드 단위 diff까지. 아무것도 쓰지 않음 | — | 중 |
| **U2** | **대시보드 창** | 없음 | 어떤 CSV가 어떤 임포터에 물려 있고, 마지막 임포트가 언제였고, 무엇이 실패했는지 한 화면 | 중 |
| ✅ **U3** | 오류의 위치 | ~~Console 텍스트뿐~~ → 리포트가 줄·열을 달고, 미리보기에 '찾기' 버튼 | — | 중 |
