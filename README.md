...
### ⚠️ 참조 조사는 Force Text 에서만 됩니다

"참조가 남은 에셋은 지우지 않는다"는 씬·프리팹·에셋을 **글자로 읽어** GUID를 찾는 방식입니다.
그래서 **Project Settings ▸ Editor ▸ Asset Serialization** 이 `Force Text` 가 아니면
GUID가 글자로 존재하지 않아 조사가 성립하지 않습니다.

그럴 때는 **표에서 사라진 산출물을 하나도 지우지 않고** 그 이유를 알립니다.
"참조가 없다"는 틀린 답을 받아 씬이 쓰는 에셋을 지우는 것보다, 지울 것이 없다고 답하는 편이 낫습니다.

정리를 쓰고 싶다면 `Force Text` 로 두십시오. git 으로 diff 와 병합이 되는 것도 그쪽입니다.
